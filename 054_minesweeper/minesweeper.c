...
board_t * makeBoard(int w, int h, int numMines) {
  ...
  int * pBoardLine = NULL;
  for (int i = 0; i < h; i++) {
    pBoardLine = malloc(w * sizeof(*pBoardLine));
    ppBoard = realloc(ppBoard, (i + 1) * sizeof(*ppBoard));
    ppBoard[i] = pBoardLine;
    pBoardLine = NULL;
  }
  //*pBoardLine **ppBOard
  //*pBoardStruc
  board_t * pBoardStruc = malloc(sizeof(*pBoardStruc));
  pBoardStruc->board = ppBoard;
  ppBoard = NULL;
  pBoardStruc->width = w;
  pBoardStruc->height = h;
  pBoardStruc->totalMines = numMines;
  for (int i = 0; i < h; i++) {
    for (int j = 0; j < w; j++) {
      pBoardStruc->board[i][j] = UNKNOWN;
    }
  }
  for (int i = 0; i < numMines; i++) {
    addRandomMine(pBoardStruc);
  }
  return pBoardStruc;
  //return NULL;
}
